...
---
This is part 3 of the guide, I will represent here the algorithm I came up with for the missiles path, how to animate trajectory and make a mesh follow the path, It’s a little math heavy 😵‍💫 but I will try to keep it as simple as possible.
...
2. Create the actual start and end points on the sphere (by multiplying the normals by the radius)
3. Calculate `arcHeight` it will be used later.
4. Quaternion logic - we calculate the `totalRotation` (between source and target) 
    - it has an edge case - if the source and target points are parallel we will set a fixed value.
5. Calculate the control points directions - we will [slerp](https://en.wikipedia.org/wiki/Slerp "https://en.wikipedia.org/wiki/Slerp") the quaternion of 25% and 75%, then apply this quaternion to source normal clone.
6. Scale those control points by multiplying the scalar `arcHeight`.

That’s it for the algorithm ! you can see it’s implementation [here](https://github.com/iLiranS/Laser-Defense-Simulation/blob/main/src/objects/missile/utils/computeArcCurve.ts "https://github.com/iLiranS/Laser-Defense-Simulation/blob/main/src/objects/missile/utils/computeArcCurve.ts").
...
- MissileManager - we will split our rendering parts to two :
    - Missiles - they will be meshes following the curve , because they have the same geometry and material for every mesh, we will use `InstancedMesh` as in the previous⁠ visuals section.
